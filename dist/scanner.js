...
function readProjectContext(targetDir) {
...
function formatMetaForSelection(metadata) {
    const parts = [];
    if (metadata.name)
    ...
    if (metadata.entryPoint)
        parts.push(`Entry point: ${metadata.entryPoint}`);
 ...
 */
export async function selectCoreFiles(scanResult, targetDir, cli, model, logDir) {
...
["src/app.ts", "src/core/engine.ts", ...]`;
    try {
        ...
        const selected = JSON.parse(arrMatch[0]);

...
function preflightChecks(targetDir, cli) {
    ...
    try {
        ...
        console.error(chalk.red(`Error: CLI "${cli}" is not installed or not in PATH.`));
        console.error(chalk.yellow(`Install it or use --cli to specify a different CLI.`));
        process.exit(1);
    }
}
...
program
    .name("smtt")
    .description("Show Me The Talk — reverse-engineer natural language descriptions from codebases")
    .version("0.1.0");
program
    .command("analyze")
    ...
    console.log(chalk.dim(`  Config: max ${options.maxRounds} rounds, threshold ${options.threshold}, CLI: ${options.cli}, mode: ${options.coreOnly ? "core-only" : "full"}`));
    if (options.resume) {
    ...
    try {
        ...
        scanSpinner.succeed(`Scanned ${scanResult.fileCount} files`);
    }
    catch (error) {
        scanSpinner.fail(`Scan failed: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
    // Setup reporter
    ...
    const verifier = new Verifier(options, reporter, {
        ...
        onSkipRound: (round, score) => {
            console.log(chalk.dim(`  Skipped (cached score: ${score}/100)`));
        },
        onSkipMember: (_round) => {
            console.log(chalk.dim("  Member skipped (generated code exists)"));
        },
        onMemberStart: (round) => {
        ...
        onComparisonComplete: (round, score, threshold) => {
            ...
            console.log(color(`  Score: ${score}/100 (threshold: ${threshold})`));
            if (score >= threshold) {
    ...
    try {
        ...
        for (const r of report.rounds) {
            ...
            console.log(`  ${icon} Round ${r.round}: ${r.score}/100 (${(r.duration / 1000).toFixed(1)}s)`);
            if (options.verbose) {
                const d = r.dimensions;
                console.log(chalk.dim(`    Structure: ${d.projectStructure}/20  Logic: ${d.coreLogic}/20  Flow: ${d.dataFlow}/20  Tech: ${d.techChoices}/20  Edge: ${d.edgeCases}/20`));
            }
        }
        ...
        console.log(passed
            ...
            : chalk.yellow.bold(`  ○ Did not pass. Best score: ${bestScore}/100 (round ${report.bestRound})`));

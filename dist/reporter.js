...
export class Reporter {
    ...
    static fromExisting(sessionDir) {
    ...
    writeTalk(talk) {
        ...
        fs.writeFileSync(path.join(this.sessionDir, filename), talk.content, "utf-8");
    }
    writeComparison(round, result) {
        const roundDir = this.ensureRoundDir(round);
        fs.writeFileSync(path.join(roundDir, "comparison.json"), JSON.stringify({
            round: result.round,
            score: result.score,
    ...
    copyGeneratedCode(round, generatedDir) {
    ...
    writeFinalReport(report) {
        fs.mkdirSync(this.sessionDir, { recursive: true });
        fs.writeFileSync(path.join(this.sessionDir, "talk-final.md"), report.finalTalk.content, "utf-8");
        fs.writeFileSync(path.join(this.sessionDir, "report.json"), JSON.stringify(report, null, 2), "utf-8");
    }
    // --- Resume helpers ---

...
function copyFiles(src: string, dest: string, files: string[]): void {
...
function buildSkeletonPrompt(talk: Talk, coreOnly: boolean): string {
...
export async function executeSkeleton(
  talk: Talk,
  cli: string,
  timeout: number,
  skeletonDir: string,
  ...
  if (talk.contentDir) addDirs.push(talk.contentDir);
...
function buildGroupPrompt(talk: Talk, group: GroupDef, coreOnly: boolean): string {
  const descSource = talk.contentDir
    ? `项目描述文件在目录：${talk.contentDir}\n请先用 Read 工具读取该目录下的所有 .md 文件，了解完整的项目描述。`
    : `项目描述如下：\n\n${talk.content}`;
...
function buildGroupFixPrompt(talk: Talk, group: GroupDef, fixCtx: MemberFixContext, coreOnly: boolean): string {
  const descSource = talk.contentDir
...
export async function executeGroup(
  ...
  if (talk.contentDir) addDirs.push(talk.contentDir);
  if (fixCtx?.reportPath) addDirs.push(path.dirname(path.resolve(fixCtx.reportPath)));
  ...
  const result = await spawnCli(
    cli, prompt, groupDir, timeout, logDir,
    `member-${group.name}`, addDirs,
  );
...
function buildPrompt(talk: Talk, coreOnly: boolean): string {
  const descSource = talk.contentDir
    ? `项目描述文件在目录：${talk.contentDir}\n请先用 Read 工具读取该目录下的所有 .md 文件，了解完整的项目描述。`
    : `项目描述如下：\n\n${talk.content}`;
...
function buildFixPrompt(talk: Talk, fixCtx: MemberFixContext, coreOnly: boolean): string {
  const descSource = talk.contentDir
    ? `项目描述文件在目录：${talk.contentDir}\n请先用 Read 工具读取该目录下的所有 .md 文件。`
    : `项目描述如下：\n\n${talk.content}`;

...
function shouldIgnore(
  ...
  for (const pattern of ignorePatterns) {
    ...
    if (clean.startsWith("*") && basename.endsWith(clean.slice(1))) return true;
  }
...
function getPriority(relativePath: string): number {
  ...
  if (basename === "package.json" || basename === "tsconfig.json") return 0;

  for (const pattern of ENTRY_PATTERNS) {
    if (pattern.test(basename)) return 1;
  }

  const inSrc = CORE_PATTERNS.some((p) => p.test(relativePath));

  for (const pattern of TEST_PATTERNS) {
    if (pattern.test(relativePath) || pattern.test(basename)) return inSrc ? 4 : 5;
  }
...
function extractMetadata(targetDir: string): ProjectMetadata {
